	...
	<div class="container mrg-top-15 mb20">
		<div class="row">
			<a href="<%=request.getContextPath()%>/message/message.do?mbNo=${mbNo}&action=outbox">
				<div class="col-xs-12 col-sm-4 btn btn-default central btn-tab-4 ">寄件匣</div>
			</a>
			<a href="#">
				<div class="col-xs-12 col-sm-4 btn btn-default central btn-tab-4 active">
					收件匣
					<c:if test="<%=inBoxMesgNotify!=0 %>">
						<span id="inBoxMesgNotify" class="badge inBoxMesgNotify" style="background-color: #5e2b97;"><%=inBoxMesgNotify %></span>
					</c:if>
				</div>
			</a>
			<a href="<%=request.getContextPath()%>/message/message.do?mbNo=${mbNo}&action=sysbox">
				<div class="col-xs-12 col-sm-4 btn btn-default central btn-tab-4">
					...
					<c:if test="<%=sysMesgNotify!=0 %>">
						<span class="badge sysMesgNotify" style="background-color: #5e2b97;"><%=sysMesgNotify %></span>
					</c:if>
				</div>
			</a>
		</div>
					...
					<c:if
						...
						<a href='#mesg-content<%=i%>' data-toggle="modal" class="">
							<div class="col-xs-12 col-sm-3 ">
								<div class="font-content">
									<!-- 寄件人姓名 -->
									<h3 class="${ receiveMailVO.mesgNo }" ${(receiveMailVO.readStatus eq '未讀')?"style='color: #b34242;
    									font-weight: bold; margin-top: 5px;'":""} style="margin-top: 5px;">
    									
    									
											${mbSvcforMailMb.getOneMember(receiveMailVO.mailMbNo).getMbLstName() }
											${mbSvcforMailMb.getOneMember(receiveMailVO.mailMbNo).getMbFstName() }
						...
						<div class="modal fade" id="mesg-content<%=i%>">
								...
								<h4 class="modal-title">
									...
									<script type="text/javascript">
											function test<%=i%>(){
												...
												var inBoxMesgNotify = $('span.inBoxMesgNotify').html();
												checkReadStatus(mesgNo,inBoxMesgNotify);								
											}
										</script>
								</h4>
								<!-- 寄件人姓名 -->
								<p>
									<a href="<%=request.getContextPath()%>/member/member.do?mbNo=${receiveMailVO.mailMbNo}&action=getMbInfo" class="purple purple-bg">
										${mbSvcforMailMb.getOneMember(receiveMailVO.mailMbNo).getMbLstName() }
										${mbSvcforMailMb.getOneMember(receiveMailVO.mailMbNo).getMbFstName() }
									</a>
								</p>
							...
							<div class="modal-body padding-l10px">
								<pre>${receiveMailVO.getMesgContent()}</pre>
							</div>
...
<!-- footer-end   ============================================================= -->
	...
	<script>
		...
		function checkReadStatus(mesgNo,inBoxMesgNotify) {
			console.log(mesgNo);
			var h3 = "."+mesgNo;
			var xhr = new XMLHttpRequest();
			var noMb;
			console.log(inBoxMesgNotify);
			/* var inBoxMesgNotify = inBoxMesgNotify;
			console.log(inBoxMesgNotify); */
			//設定好回呼函數   
			 xhr.onreadystatechange = function() {
				if (xhr.readyState == 4) {
					if (xhr.status == 200) {
						...
						$("span.inBoxMesgNotify").html(inBoxMesgNotify-1);
						console.log(inBoxMesgNotify);
						if(inBoxMesgNotify==1){
							console.log(inBoxMesgNotify);
							$("#inBoxMesgNotify").remove();
						}
						console.log(inBoxMesgNotify);
					}//xhr.status == 200
				}//xhr.readyState == 4
		...
		function checkRecipentIsNull() {
			...
			//建立好Get連接
			var url = "message.do?action=checkMbExist&recipient=" + $("#recipient").val();
			xhr.open("Get", url, true);
			//送出請求 
			xhr.send(null);
			console.log(url);
