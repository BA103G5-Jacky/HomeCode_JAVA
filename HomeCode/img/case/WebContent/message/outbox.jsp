...
<%-- 						class="com.member.model.MemberService" /> --%>
							...
							<a href="message.do?action=changeReadStatus&readStatus=read&mesgNo=${sendMailVO.mesgNo }&servletPath=<%=request.getServletPath()%>"><div class="col-xs-12 col-sm-3 "  >
...
// 								checkReadStatus(mesgNo);								
							...
							<p>寄給：${mbSvcforMailMb.getOneMember(sendMailVO.mailRecvMbNo).getMbLstName() }
								${mbSvcforMailMb.getOneMember(sendMailVO.mailRecvMbNo).getMbFstName() }</p>
						</div>
						
						

						<div class="modal-body padding-l10px">${sendMailVO.getMesgContent()}</div>
