...
public class TimelineDAO implements TimelineDAO_interface {
	...
	private static final String INSERT_STMT_with_file = "INSERT INTO timeline (noteNo, noteTitle, recordDate, noteContent,csNo, uploadTime, fileName, filePath ) VALUES ('TLNO'||lpad(timeline_sq.nextval, 7, '0'),?,?,?,?,systimestamp,?,?)";
	// check
	...
	// check
	private static final String GET_ONE_STMT = "SELECT noteNo,noteTitle,noteContent,to_char(uploadTime,'yyyy-MM-dd HH:MI:ss') uploadTime, to_char(recordDate,'yyyy-mm-dd') recordDate, fileName, filePath, CSNO  FROM timeline where csNo = ?  order by recordDate desc, NOTENO desc";
