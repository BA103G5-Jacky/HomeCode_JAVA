...
public class MemberServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if ("login".equals(action)) {
			...
			if(session.getAttribute("signSuccess") != null){
				session.removeAttribute("signSuccess");
			}
		...
		if ("signup".equals(action)) {
			...
			if (lastname.length() == 0 || lastname.contains(" ") || lastname.length() > 13) {
				...
				mbVO.setMbLstName(lastname);
			}
			
			String mbUserName = req.getParameter("mbUserName");
			if (mbUserName.trim().length() == 0 || mbUserName == null) { // 判斷ㄊ他有沒有打字
				errorMsgs.add("會員暱稱不可以空白");
			} else if( mbSvc.getAllMbUserName().contains(mbUserName)) {
				errorMsgs.add("會員暱稱已經有人使用");
			} else {
				mbVO.setMbUserName(mbUserName);
			}
			
			
			String email = req.getParameter("email"); // 驗證信箱

			if (email.trim().length() == 0 || email == null) { // 判斷ㄊ他有沒有打字
				errorMsgs.add("Email不可以空白!");
			} else if (!email.contains("@")) { // 判斷email那邊它是不是輸入信箱
				errorMsgs.add("email格示不正確!");
			} else if (mbSvc.emailCheck().contains(email)) {
			...
			// 如果上面有發生任何一個錯誤的話!
			if (!errorMsgs.isEmpty()) {
				req.setAttribute("mbVO", mbVO);
				RequestDispatcher failureView = req.getRequestDispatcher("/signup/visitor_signup.jsp");
				failureView.forward(req, res);
				return;
			}
			...
			MemberVO memberVO = mbSvc.addMember(lastname, firstname, null, email, password, location, null, null,
					mbUserName, null, null);
			session.setAttribute("signSuccess", "註冊成功歡迎登入");
