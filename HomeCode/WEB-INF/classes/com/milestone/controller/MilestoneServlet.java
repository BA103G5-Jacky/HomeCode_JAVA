...
public class MilestoneServlet extends HttpServlet{
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res)
		...
		System.out.println("Milestone - action = "+action);
		...
		if ("agree-milestone".equals(action)){ //來自milestone.jsp的請求
			...
			if("已完成".equals(milestoneState)){
				...
				//timelineVO.setRecordDate(new java.sql.Date(new java.util.Date().getTime()));
				 ...
				 //*********only for project presentation 只給專案展示時使用*********//
			    if(milestoneVO.getMilestoneName().equals("前端功能")){
			     timelineVO.setRecordDate(Date.valueOf("2017-10-30"));
			    }else if(milestoneVO.getMilestoneName().equals("後端功能")){
			     timelineVO.setRecordDate(Date.valueOf("2017-11-15"));
			    }else if (milestoneVO.getMilestoneName().equals("前端網站改版2.0")){
			     timelineVO.setRecordDate(Date.valueOf("2017-11-30"));
			    }else{
			     timelineVO.setRecordDate(new java.sql.Date(new java.util.Date().getTime()));
			    }
			    //*********only for project presentation 只給專案展示時使用*********//
			...
			if(req.getSession().getAttribute("android") != null){
				...
				url = "/android/AndroidServlet?action=milestone_Android&csNo="+csNo+"&mbNo="+mbNo;
			} else{
				url = "/milestone/milestone.do?action=getCase_milestone&csNo=" + csNo;
			}
...
//			res.sendRedirect(url);
		...
		if("deleteMilestone".equals(action)){
			
			String milestoneNO =  req.getParameter("milestoneNO");
			System.out.println("milestoneNO = " + milestoneNO);
			String csNo = req.getParameter("csNo");
			System.out.println("csNo = " + csNo);
			MilestoneService milestoneSvc = new MilestoneService();
			milestoneSvc.deleteEmp(milestoneNO);
			...
			if(req.getSession().getAttribute("android") != null){
				...
				req.getSession().removeAttribute("android");
				String mbNo = (String) req.getSession().getAttribute("mbNo");
				url = req.getContextPath() + "/android/AndroidServlet?action=milestone_Android&csNo="+csNo+"&mbNo="+mbNo;
			} else{
				url = req.getContextPath() + "/milestone/milestone.do?action=getCase_milestone&csNo=" + csNo;
			}
			res.sendRedirect(url);
		}
		...
		if("insertAfter".equals(action)){
			...
			System.out.println(milestoneNameList);
			
			if(milestoneNameList != null && startdateList != null && enddateList != null){
				
				//比對是否有預設的案件完成 有的話刪掉
				List<MilestoneVO> milestoneList = milestoneSvc.getMileStoneByCs(csNo);
				for(MilestoneVO vo : milestoneList){
					if("案件完成".equals(vo.getMilestoneName())){
						milestoneSvc.deleteEmp(vo.getMilestoneNO());
					}
				}
				...
				for(int i = 0; i<milestoneNameList.length; i++){
					Date startDate = null;
					Date endDate = null;
					if(milestoneNameList[i].trim().length() == 0){
						milestoneNameList[i] = "進度里程" + (i+1);
					}
					if( startdateList[i].trim().length() == 0){
						startDate = new java.sql.Date(new java.util.Date().getTime());
					}
					if( enddateList[i].trim().length() == 0){
						endDate = new java.sql.Date(new java.util.Date().getTime());
					}
					if(startdateList[i].contains("/")){
						try{
