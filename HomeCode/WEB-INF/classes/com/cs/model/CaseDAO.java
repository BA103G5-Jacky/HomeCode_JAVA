...
public class CaseDAO implements CaseDAO_interface{
	...
	public final static String UPDATE_CSPAYSTATE_STMT = "UPDATE CASE SET CSPAYSTATE = ? WHERE CSNO = ?";
	...
	public CaseVO findByPrimaryKey(String csNo) {
		...
		try {
			...
			while (rs.next()) {
				...
				caseVO.setPostTime(rs.getTimestamp("postTime"));
	...
	public void updateCsPayState(String csNo) {
		...
		try {
			con = ds.getConnection();
			pstmt = con.prepareStatement(UPDATE_CSPAYSTATE_STMT);
			
			pstmt.setString(1, "已付款");
			pstmt.setString(2, csNo);

			int a = pstmt.executeUpdate();
			System.out.println("[CaseDAO-UpdateCsPayState] 已更新" + a + "筆資料");
		} catch (SQLException se) {
			throw new RuntimeException("A database error occured. " + se.getMessage());
		} finally {
			if (rs != null) {
