...
public class Case_CompositeQuery {
	...
	public static String get_aCondition_For_Oracle(String columnName, String value) {
		...
		} else if ("searchInput".equals(columnName)){
			aCondition = "csLoc like '%" + value + "%' or csname like '%" + value + "%' or csDesc like '%" + value
					+ "%' ";
		}
...
//			aCondition = "to_char(" + columnName + ",'yyyy-mm-dd')='" + value + "'";
	...
	public static String get_Condition_From_SkillNo(String columnName, String value){
		...
		condition = "csNo in ( select csNo from Case_Type where " + columnName + " = '" + value +"')";
		return condition + " ";
	}
	...
	public static String get_WhereCondition(Map<String, String[]> map) {
		...
		for (String key : keys) {          //把所有參數map的key都取出來
			...
			if (value != null && value.trim().length() != 0	&& !"action".equals(key) && !"skillNo".equals(key)) { //檢查是否為空字串or null
