...
// //Case
		CaseService csSvc = new CaseService();
 		...
 		if(request.getAttribute("doneCsList") == null){
			List<CaseVO> doneCsList =csSvc.findByHirerNo((String)session.getAttribute("mbNo"),"已完成");
...
<html lang="">
	<head>
		...
		<link rel="stylesheet" type="text/css" href="<%=request.getContextPath()%>/case/css/doneByHirer.css">
		<link rel="stylesheet" type="text/css" href="<%=request.getContextPath()%>/case/css/allCase.css">
...
<div class="container mrg-top-15 mb20">
	<div class="row">
		...
		<a href="<%=request.getContextPath()%>/case/case.do?mbNo=${mbNo}&action=doneCase&actor=hirer"><div class="col-xs-12 col-sm-4 btn btn-default central btn-tab-4 active">已完成案件</div></a>               
	</div> <!-- Row End -->
						...
						<div class="modal-dialog">
							<div class="modal-content rate">
								<div class="modal-header">
									...
									<h4 class="modal-title">評價接案人</h4>
								</div>
								<div class="modal-body">
									<legend>接案人 -${mbSvc.getOneMember(cs.freelancerNo).mbLstName}${mbSvc.getOneMember(cs.freelancerNo).mbFstName}</legend>
									
									 <fieldset class="starability-basic">
									      <legend>評價分數</legend>
									
									      <input type="radio" id="rate1" name="freelancerScore" value="1" />
									      <div class="glyphicon glyphicon-star"></div>
									
									      <input type="radio" id="rate2" name="freelancerScore" value="2" />
									      <div class="glyphicon glyphicon-star"></div>
									      <div class="glyphicon glyphicon-star"></div>
									
									      <input type="radio" id="rate3" name="freelancerScore" value="3" />
									      <div class="glyphicon glyphicon-star"></div>
									      <div class="glyphicon glyphicon-star"></div>
									      <div class="glyphicon glyphicon-star"></div>
									
									      <input type="radio" id="rate4" name="freelancerScore" value="4" />
									      <div class="glyphicon glyphicon-star"></div>
									      <div class="glyphicon glyphicon-star"></div>
									      <div class="glyphicon glyphicon-star"></div>
									      <div class="glyphicon glyphicon-star"></div>
									
									      <input type="radio" id="rate5" name="freelancerScore" value="5" />
									      <div class="glyphicon glyphicon-star"></div>
									      <div class="glyphicon glyphicon-star"></div>
									      <div class="glyphicon glyphicon-star"></div>
									      <div class="glyphicon glyphicon-star"></div>
									    ...
									    <legend>評價內容</legend>
									    <textarea class="form-control" rows="5" cols="20" name="freelancerComment" id="comment" placeholder="請輸入50字以內的評論"></textarea>
								</div>
								<div class="modal-footer">
									<input type="hidden" name="csNo" value="${cs.csNo}">
									<input type="hidden" name="action" value="updateFreelancerScore">
									<!-- magic button -->
...
<!-- footer區結束 =========================================================== -->
		...
		<script>
			window.onload = function(){
				for(var i=0; i<${doneCsList.size()}; i++){
					<c:forEach var="vo" items="${doneCsList}" varStatus="i">
						showStarsList(${vo.hirerScore}, ${i.index});
					</c:forEach>
				}
			...
			function rate(i){
				for(var j = 0; j<5;j++){
					document.getElementsByName("freelancerScore")[j].removeAttribute("checked");
					if( j == (i-1)){
						document.getElementsByName("freelancerScore")[j].setAttribute("checked","");
					}
				}
		...
		<script>
			function inputInfo1(){
				...
				document.getElementById('comment').innerHTML = "幹得不錯，下星期一起來上節目";
