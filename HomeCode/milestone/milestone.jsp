...
<c:if test="${csVO.csState eq '進行中' && mbNo eq csVO.hirerNo}">
	...
	<div class="container mrg-top-15 mb20">
		<div class="row">
			<a href="<%=request.getContextPath()%>/case/case.do?csNo=${csVO.csNo}&action=caseInfo">
				<button type="button" class="col-xs-12 col-sm-4 btn btn-default central btn-tab-4" id="caseInfo">案件詳情</button>
			</a>
			<c:if test="${mbNo.equals(csVO.hirerNo) || mbNo.equals(csVO.freelancerNo)}">
				<a href="<%=request.getContextPath()%>/milestone/milestone.do?action=getCase_milestone&csNo=${csVO.csNo}">
					<button type="button" class="col-xs-12 col-sm-4 btn btn-default central btn-tab-4 active" id="caseMilestone">案件進度</button>
				</a>
				<a href="<%=request.getContextPath()%>/timeline/timeline.do?csNo=${csVO.csNo}&action=getCaseTimeline">
							...
							<div class="panel panel-default">
								...
								<div class="col-sm-8  img-va">
									...
									<c:if test="${mbNo eq csVO.freelancerNo }">
										<c:if test="${'已完成' eq milestoneListReverse[i.index+1].milestoneState && '未完成' eq milestoneVO_r.milestoneState}">
